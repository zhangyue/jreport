...
import pers.yue.jreport.exceptions.runtime.InvalidFormatRuntimeException;

import static pers.yue.jreport.util.FileUtil.*;
...
public class StringUtil {
    ...
    public static long parseSizeStr(String sizeStr) {
        if (sizeStr.matches("\\d+GB")) {
            return StringUtil.toLong(sizeStr.replace("GB", "")) * GB;
        } else if (sizeStr.matches("\\d+MB")) {
            return StringUtil.toLong(sizeStr.replace("MB", "")) * MB;
        } else if (sizeStr.matches("\\d+KB")) {
            return StringUtil.toLong(sizeStr.replace("KB", "")) * KB;
